...
export async function openStack({ root, kind }) {
  ...
  return {
    ...
    async list(revset) {
      ...
      const out = await runGit(['-C', root, 'rev-list', ...walk, revset, '--']);
      return parseListing(out.replace(/^(\S+)$/gm, (sha) => `${sha} ${sha.slice(0, 12)}`));
    },
...
/** `<sha> <label>` per line, as both listings are made to produce. */
