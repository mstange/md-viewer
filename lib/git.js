...
/**
 * Run git with these arguments and resolve to its output. The caller says
 * which repository, with `-C` or `--git-dir`, since the two ways this module
 * is used find it differently: from a file inside a checkout here, and from a
 * jj repository's git store in stack.js.
 */
export function runGit(args, { maxBuffer = 1024 * 1024 } = {}) {
...
function git(repo, args, options) {
  return runGit(['-C', repo, ...args], options);
}

/**
 * The `git show` that produces a commit as the diff renderer reads it.
 *
 ...
 *
 * Patience rather than git's default Myers. Myers minimises the edit count,
 * and is happy to delete a whole function and add two where patience — which
 * anchors on the lines unique to both sides, such as a function signature —
 * keeps the signature that survived as context and shows the new function as
 * one insertion. That is what `jj show` writes for the same commit (git's own
 * histogram, tried first, is not), so a stack reads the same in the viewer as
 * on the command line.
 */
...
export function openRepository(file) {
  ...
  return {
    ...
    async show(sha) {
      ...
      return git(repo, showArgs(sha), { maxBuffer: MAX_PATCH_BYTES });
    },
