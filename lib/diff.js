...
function pairRun(dels, adds) {
  ...
  for (let i = paired; i < dels.length; i++) {
    pairs.push({ del: dels[i], add: null, delHtml: escape(dels[i].text), space: false });
  }
  for (let i = paired; i < adds.length; i++) {
    pairs.push({ del: null, add: adds[i], addHtml: escape(adds[i].text), space: false });
  }
  return pairs;
}
 ...
 * grid placements of the same elements, which matters because the same text can
 * appear only once in the page — a second copy would make every review anchor
 * match twice and double the quote that gets copied out.
 *
...
function rowHtml(row) {
  if (row.type === 'context') {
    return (
      ...
      lineHtml('ctx', row.oldLine, row.newLine, escape(row.text)) +
      '</div>'
    );
  }
