 ...
 * would pair the rewritten line with the first insertion and mark both as
 * heavily changed, when the line it is really a version of is further down. So
 * candidates are scored against each other and the best mutual matches win,
 * with anything too dissimilar left unpaired rather than forced.
 *
 * @returns {Map<number, number>} index in `dels` to index in `adds`
 */
function matchLines(dels, adds) {
  const scores = [];
  for (let d = 0; d < dels.length; d++) {
    for (let a = 0; a < adds.length; a++) {
      const score = similarity(diffWordsWithSpace(dels[d].text, adds[a].text));
  ...
  for (const { d, a } of scores) {
    if (byDel.has(d) || takenAdds.has(a)) {
      continue;
    }
    byDel.set(d, a);
    takenAdds.add(a);
...
function pairRun(dels, adds) {
  // Pairing is for edits: a line that was tweaked, next to the line it became.
  // A run that replaces two lines with thirteen is not that — it is a block
  // rewritten, and reflowing prose across new line breaks moves text between
  // lines until no pairing is better than a coin toss. Pairing one anyway
  // invents a correspondence that is not there, and highlights the words two
  // unrelated lines happen to share. So only a run that swaps lines one for one
  // is paired; anything lopsided is shown as what it is, lines gone and lines
  // arrived.
  //
  // Matching is quadratic in the size of the run, so a pathological one falls
  // back to the same plain treatment rather than hanging the render.
  const evenSwap =
    dels.length === adds.length && dels.length * adds.length <= MAX_PAIRED_CELLS;
  const matched = evenSwap ? matchLines(dels, adds) : new Map();

  // A line with no counterpart is new or gone in its entirety, whatever else
  // the run is doing. It is marked as a whole line rather than by highlighting
  // its text: a mark that hugs the glyphs stops at the last character, which
  // reads as a highlighter drawn over the words. What changed is the line, so
  // the colour is the line's.

  const pairs = [];
  const usedAdds = new Set(matched.values());
  let next = 0;
  ...
  for (let d = 0; d < dels.length; d++) {
    ...
    pairs.push({
      del: dels[d],
      add: adds[a],
      delHtml: markUp(parts, 'del', space),
