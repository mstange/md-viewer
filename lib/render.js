 ...
 * block in the file. `first` is the line `source` starts on in the real file,
 * which is past 1 when a frontmatter block was split off. Returns null if the
 * total does not add up, since a preview with no line numbers beats one with
 * wrong ones.
 */
 ...
 */
export function renderMarkdown(source, { baseDir, linkMode = 'server', commits = null } = {}) {
